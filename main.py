...
async def ask_groq(question: str, subject: str = "Математика", exam: str = "ОГЭ") -> str:
    try:
...
async def menu_callback(update: Update, context):
    ...
    if query.data == "menu_subjects":
        ...
        exam = data.get("exam", "ОГЭ")
        reply = await ask_groq(f"Дай случайное задание по {subject} для {exam}.", subject, exam)
...
async def handle_text(update: Update, context):
    ...
    reply = await ask_groq(text, subject, exam)
